...
bin/mutant                 # Mutation testing, whole app in stages (hours, not in bin/check; docs/mutation-testing.md)
bin/storm                  # Real-server concurrency storm: many Puma threads, many clients (not in bin/check; docs/concurrency-testing.md)
```
...
8. **No denormalized counters or caches for financial data.** The `counter_culture` gem has been removed entirely. All derived values (costs, counts, multiplier sums) are computed from source data via SQL queries or Ruby enumeration. There are exactly two caches. `resident_balances` is rebuilt daily from source data. The calendar month is cached for one hour in `CommunitiesController#calendar` under a version read from the rows (`Community#calendar_cache_version`: the row count and newest `updated_at` of every table the month is drawn from, plus today's date), so a write through a model is a miss for the next reader even when it lands while another request is still building the month — deleting the entry cannot promise that (`spec/requests/api/v1/calendar_cache_race_spec.rb`). The list of tables in the version and of the model callbacks that push is at the top of `app/serializers/calendar_serializer.rb`. **A cache entry must come with a written list of every write that changes its data, and each of those writes must clear it.** If you cannot write that list, do not cache. Pushing is the models' job, not a controller's: every save and destroy callback that changes what a screen shows notes itself in `LiveUpdate` (`app/services/live_update.rb`), which clears once after the transaction commits and hands each push to `LivePushJob`, so admin forms, rake tasks and settlements push the same as the API and no request waits on Pusher (ADR 0007, `spec/requests/api/v1/live_update_contract_spec.rb`). Live updates therefore need the Solid Queue supervisor running, like the nightly jobs. This is how #76 happened: the cooks form was cached per meal, only that meal's own writes cleared it, and a renamed or retired resident stayed on the form for a day. Fresh queries are the default. A cache has to show a measured cost before it is added, and the list of clearing writes is part of the change.
...
- **prosopite fails a spec that runs an N+1 query.** It watches every request (a Rack middleware in the test environment, one scan per request) and every example under `spec/services` and `spec/serializers`, and raises when the same SQL shape runs again and again from the same Ruby line. Factories run unwatched, and an example tagged `prosopite: false` is unwatched too; say why next to the tag. Fix the repeat with a preload or one query for the set. When the repeat is right (one lock per row write, one audit read per audited write), add the file or a regexp on the stack line to the allow list in `spec/support/prosopite.rb` with the reason. Bullet stays in development only: it shows a page footer but cannot fail a spec.
- **Sorbet type checks the Ruby side.** `bin/check` runs `srb tc`. Models (concerns included), services, serializers, controllers, jobs, mailers, helpers and `lib` are `# typed: true`; the money path is `# typed: strict`; admin, config, db are `# typed: false`; `spec/` is ignored. Column readers are `T.nilable` on purpose, even NOT NULL ones, because validations run on records that hold nil. After a migration or a new association run `bin/tapioca dsl`; after a gem update run `bin/tapioca gem`. Sigs are checked at runtime everywhere, production included, and raise: a wrong value must not reach the ledger. The rules and the reasons: `docs/sorbet.md`.
...
- **FallbackController serves the SPA, and index.html stays in `public/`.** One build layout for every path: `config.public_file_server.index_name` names a file that never exists, so the static server never resolves "/" to index.html and the router's subdomain constraints stay in charge — FallbackController is the only thing that serves it. (The build used to move index.html to `app/frontend/dist/`, which gave the repo two layouts, #52.) The static file server serves `.vite/manifest.json` like any other file in `public/` (a dotfile directory is no exception; a controller action for it was dead code until 2026-09-08), and `AssetCacheControl` marks it `no-cache` so the version banner sees a deploy.
...
- **Concurrency is tested at four layers** (`docs/concurrency-testing.md`): a Prism scan refuses class variables, `Thread.current` and class-level memos, and the `rubocop-thread_safety` cops (in the RuboCop step) refuse writes to class attributes, mutable class-level objects, `Dir.chdir` and middleware instance variables; a reused-thread spec proves `Current` is empty at the start of every request and job; an in-process storm of concurrent requests with a settlement, the nightly jobs and an unlocked admin beside it runs in `bin/check` (`spec/concurrency/`); and `bin/storm` does the same against a real multi-threaded Puma. Every write path that reads and then writes at SERIALIZABLE must retry a conflict and answer 409 when it keeps happening: the meal writes through `with_meal_lock`, the calendar writes through `ApiController#render_retrying_on_conflict`, the recurring jobs through `RecurringJob`. Reads are retried where they are built (the calendar month) and `ApiController` rescues any conflict that escapes as a 409, never a 500. A write to a meal's rows takes the meal's lock first, from the model (`LocksItsMealFirst`), so the API, admin, a task and the console all lock a meal and its rows in the same order — the database trigger locks them the other way round, and two orders is a deadlock.
