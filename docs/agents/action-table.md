...
| Move a meal to another date                       | —                                                                                              | **THIN**: `live_update_contract_spec` proves the push; no admin request spec that the date saves and the old month drops it | —                                                            | —                                                    |
...
| Action                       | API                                                                        | Admin                                                                                                                                                  | Task/job                                                                                                                                                                               | Browser |
| ---------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| Preview a settlement         | `reconciliations_preview_spec`, `reconciliations_authorization_spec`       | **EMPTY**: the admin "new reconciliation" form shows no preview; if it should, that is a feature, not a hole                                           | —                                                                                                                                                                                      | —       |
...
| Refresh balances             | —                                                                          | —                                                                                                                                                      | `billing_recalculate_spec`, `_correctness_spec`, `_snapshot_spec`; `RefreshBalancesJob` itself: **THIN**, only `recurring_job_spec` (the wrapper), no spec that the job calls the task | —       |
...
| Create a guest-room reservation            | `guest_room_reservations_controller_spec` (duplicate date) | **THIN**: `permit_params_smoke_spec` saves it; nothing shows the duplicate-date refusal renders as an error, not a 500 | —        | `guest room lifecycle`   |
| Update / delete a guest-room reservation   | `..._controller_spec`, `high_trust_authorization_spec`     | **EMPTY**                                                                                                              | —        | `guest room lifecycle`   |
| Create a common-house reservation          | `common_house_reservations_controller_spec` (overlap)      | **THIN**: as guest room, for the overlap refusal                                                                       | —        | `common house lifecycle` |
| Update / delete a common-house reservation | `..._controller_spec` (bad dates → 400)                    | **EMPTY**                                                                                                              | —        | `common house lifecycle` |
...
| Rename a resident                                        | —                                                         | **THIN**: `superuser_authorization_spec` renames as a side effect; the cache effect is in `communities_controller_spec` and `stale_meal_form_cache_spec`, both through the model, not the form   | —                                                                         | —                                      |
| Retire a resident (`active: false`)                      | —                                                         | **EMPTY** as a form submission; cache effect through the model only (#77)                                                                                                                        | —                                                                         | —                                      |
| Change a resident's multiplier or price category by hand | —                                                         | **EMPTY**: `permit_params_smoke_spec` saves the field; nothing proves a settled `meal_residents.multiplier` snapshot stays put when this form changes the resident (the task path has that spec) | `residents_set_multiplier_spec` (boundaries, settled snapshots untouched) | —                                      |
| Move a resident to another unit                          | —                                                         | **EMPTY**                                                                                                                                                                                        | —                                                                         | —                                      |
...
| Log in / log out                                         | `sessions_controller_spec`, `residents_controller_spec`   | `admin_smoke_spec`, `admin_logout_spec`, `session_persistence_spec`                                                                                                                              | —                                                                         | `login` tests, `logout clears cookies` |
| Create a unit                                            | —                                                         | `permit_params_smoke_spec`                                                                                                                                                                       | —                                                                         | —                                      |
| Rename a unit                                            | —                                                         | **THIN**: cache effect through the model in `communities_controller_spec`; no form submission                                                                                                    | —                                                                         | —                                      |
...
| Action                                    | API                         | Admin                                                                                                                                                                                                                                                                                         | Task/job                                                                                         | Browser          |
| ----------------------------------------- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------- |
| Create a rotation                         | —                           | **THIN**: `rotation_destroy_spec` renders the new form; nothing submits it                                                                                                                                                                                                                    | `community_create_rotations_spec`; `EnsureRotationsJob` itself only through `recurring_job_spec` | —                |
| Edit a rotation (description, `meal_ids`) | —                           | **EMPTY**. This is #78: `meal_ids=` on the edit form destroys the rotation's existing meals. The issue is closed as completed, but `app/admin/rotation.rb:5` still permits `meal_ids`, no commit names a fix, and no spec submits this form. Treat it as open until a red spec says otherwise | —                                                                                                | —                |
...
| Change dinner start times                 | —                           | **THIN**: DB check in `dinner_start_times_check_spec`; model in `community_dinner_start_times_spec` (DST); no admin form submission, and no spec that the calendar and iCal show the new time after the change                                                                                | —                                                                                                | —                |
| Change the time zone                      | —                           | **EMPTY**: nothing submits it, and nothing shows what happens to "today" and to the meal chips on the day it changes                                                                                                                                                                          | —                                                                                                | —                |
...
| Turn broadcast email on or off              | —                          | **EMPTY** as a form submission; the tasks honor the flag (`residents_notify_spec`, `rotations_notify_new_spec`)        | —                               | —       |
...
1. Admin move of a meal's date: request spec, old month drops it.
2. Admin rotation edit (#78): a red spec, then a fix or a decision that
   the form should not offer `meal_ids` at all.
3. `RefreshBalancesJob` and `EnsureRotationsJob`: one spec each that
   the job runs its task.
4. Admin resident form: rename, retire, change multiplier, move unit —
   each as a form submission, with the settled snapshot and the caches
   checked afterward.
5. Admin reservation update and delete, and the overlap refusal
   rendering as an error.
6. Admin unit rename as a form submission.
7. Admin rotation create as a form submission.
8. Dinner start time change: calendar and iCal show the new time.
9. Time zone change: what "today" does on the day it changes.
10. Broadcast email flag as a form submission.
