...
2. `RefreshBalancesJob` and `EnsureRotationsJob`: one spec each that
   the job runs its task.
3. Admin resident form: rename, retire, change multiplier, move unit —
   each as a form submission, with the settled snapshot and the caches
   checked afterward.
4. Admin reservation update and delete, and the overlap refusal
   rendering as an error.
5. Admin unit rename as a form submission.
6. Dinner start time change: calendar and iCal show the new time.
7. Time zone change: what "today" does on the day it changes.
8. Broadcast email flag as a form submission.
