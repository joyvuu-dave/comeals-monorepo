...
| Test-server ports           | **Namespaced** (#65). The worktree's `.env` sets `TEST_PORT_INTEGRATION`, `TEST_PORT_E2E`, and `TEST_PORT_ADMIN_E2E` — one block of three from 21000–21999, picked by a hash of the task name and checked against sibling worktrees' `.env` files. `bin/test-integration`, both Playwright configs (`tests/helpers/ports.js`), and `tests/admin/env.sh` (the admin server and its seed reload) all read them. The main checkout and CI set none of them and keep 3001/3037/3038.                           |
| Development database        | **Policy.** Worktrees never migrate or write `comeals_development`. New migrations run against the worktree's test database only (`RAILS_ENV=test rails db:migrate`). `bin/check` knows: in a worktree its migration check looks at the test database. `bin/tapioca dsl` boots development and refuses while a migration is pending there, so after a migration run it as `DATABASE_URL=postgresql:///comeals_test_<task> bin/tapioca dsl` (docs/sorbet.md); `bin/check` does the same for its verify run. |
| Admin e2e database          | **Namespaced.** `tests/admin/env.sh` appends `TEST_DB_SUFFIX`, so a worktree's admin suite uses `comeals_admin_e2e_<task_name>`. (It used to share one database, serialized by the fixed port 3038; per-worktree ports removed that serialization, so the database is namespaced like the test database.)                                                                                                                                                                                                  |
