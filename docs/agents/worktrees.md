...
| Development database        | **Policy.** Worktrees never migrate or write `comeals_development`. New migrations run against the worktree's test database only (`RAILS_ENV=test rails db:migrate`). `bin/check` knows: in a worktree its migration check looks at the test database. `bin/tapioca dsl` boots development and would refuse while a migration is pending there, so in a worktree the wrapper points it at the test database itself (docs/sorbet.md).                                             |
