...
5. New frontend code is written in `.ts` / `.tsx` by default. Existing `.js` / `.jsx` files are converted opportunistically when touched, never as a migration project. Since 2026-08-26 every `.ts`/`.tsx` file is linted by typescript-eslint (`npm run lint`, so `bin/check`, CI and the pre-commit hook all run it); before that they were type-checked but never linted. TypeScript is pinned to the 6.x line for it: the native 7.x compiler has no JavaScript API and typescript-eslint cannot parse with it yet. Since 2026-08-25 `bin/check` enforces the first half: the remaining `.js` / `.jsx` files are listed in `docs/agents/frontend-js-files.txt`, a file not on the list fails the check, and a converted file is taken off it, so the list only shrinks.
