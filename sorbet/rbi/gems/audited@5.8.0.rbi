# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `audited` gem.
# Please instead update this file by running `bin/tapioca gem audited`.


# pkg:gem/audited#lib/audited.rb:5
module Audited
  class << self
    # pkg:gem/audited#lib/audited.rb:21
    def audit_class; end

    # pkg:gem/audited#lib/audited.rb:19
    def audit_class=(_arg0); end

    # remove audit_model in next major version it was only shortly present in 5.1.0
    #
    # pkg:gem/audited#lib/audited.rb:29
    def audit_model(*args, **_arg1, &block); end

    # pkg:gem/audited#lib/audited.rb:12
    def auditing_enabled; end

    # pkg:gem/audited#lib/audited.rb:12
    def auditing_enabled=(_arg0); end

    # pkg:gem/audited#lib/audited.rb:37
    def config; end

    # pkg:gem/audited#lib/audited.rb:12
    def current_user_method; end

    # pkg:gem/audited#lib/audited.rb:12
    def current_user_method=(_arg0); end

    # pkg:gem/audited#lib/audited.rb:12
    def ignored_attributes; end

    # pkg:gem/audited#lib/audited.rb:12
    def ignored_attributes=(_arg0); end

    # pkg:gem/audited#lib/audited.rb:12
    def ignored_default_callbacks; end

    # pkg:gem/audited#lib/audited.rb:12
    def ignored_default_callbacks=(_arg0); end

    # pkg:gem/audited#lib/audited.rb:12
    def max_audits; end

    # pkg:gem/audited#lib/audited.rb:12
    def max_audits=(_arg0); end

    # pkg:gem/audited#lib/audited.rb:33
    def store; end

    # pkg:gem/audited#lib/audited.rb:12
    def store_synthesized_enums; end

    # pkg:gem/audited#lib/audited.rb:12
    def store_synthesized_enums=(_arg0); end
  end
end

# pkg:gem/audited#lib/audited/audit.rb:42
class Audited::Audit < ::ActiveRecord::Base
  include ::Audited::Audit::GeneratedAttributeMethods
  include ::Audited::Audit::GeneratedAssociationMethods
  include ::Kaminari::ActiveRecordModelExtension
  include ::Kaminari::ConfigurationMethods
  extend ::Kaminari::ConfigurationMethods::ClassMethods

  # pkg:gem/audited#lib/audited/audit.rb:47
  def _run_create_callbacks(&block); end

  # pkg:gem/audited#lib/audited/audit.rb:43
  def _run_save_callbacks(&block); end

  # Return all audits older than the current one.
  #
  # pkg:gem/audited#lib/audited/audit.rb:69
  def ancestors; end

  # pkg:gem/audited#lib/audited/audit.rb:49
  def audited_class_names; end

  # pkg:gem/audited#lib/audited/audit.rb:49
  def audited_class_names=(val); end

  # pkg:gem/audited#lib/audited/audit.rb:45
  def autosave_associated_records_for_associated(*args); end

  # pkg:gem/audited#lib/audited/audit.rb:43
  def autosave_associated_records_for_auditable(*args); end

  # pkg:gem/audited#lib/audited/audit.rb:44
  def autosave_associated_records_for_user(*args); end

  # Returns a hash of the changed attributes with the new values
  #
  # pkg:gem/audited#lib/audited/audit.rb:83
  def new_attributes; end

  # Returns a hash of the changed attributes with the old values
  #
  # pkg:gem/audited#lib/audited/audit.rb:90
  def old_attributes; end

  # Return an instance of what the object looked like at this revision. If
  # the object has been destroyed, this will be a new record.
  #
  # pkg:gem/audited#lib/audited/audit.rb:75
  def revision; end

  # Allows user to undo changes
  #
  # pkg:gem/audited#lib/audited/audit.rb:97
  def undo; end

  # pkg:gem/audited#lib/audited/audit.rb:130
  def user; end

  # pkg:gem/audited#lib/audited/audit.rb:123
  def user=(user); end

  # pkg:gem/audited#lib/audited/audit.rb:129
  def user_as_model; end

  # pkg:gem/audited#lib/audited/audit.rb:122
  def user_as_model=(value); end

  # @private
  #
  # pkg:gem/audited#lib/audited/audit.rb:126
  def user_as_string; end

  # Allows user to be set to either a string or an ActiveRecord object
  # @private
  #
  # pkg:gem/audited#lib/audited/audit.rb:115
  def user_as_string=(user); end

  private

  # pkg:gem/audited#lib/audited/audit.rb:187
  def set_audit_user; end

  # pkg:gem/audited#lib/audited/audit.rb:198
  def set_remote_address; end

  # pkg:gem/audited#lib/audited/audit.rb:193
  def set_request_uuid; end

  # pkg:gem/audited#lib/audited/audit.rb:177
  def set_version_number; end

  class << self
    # All audits made during the block called will be recorded as made
    # by +user+. This method is hopefully threadsafe, making it ideal
    # for background operations that require audit information.
    #
    # pkg:gem/audited#lib/audited/audit.rb:140
    def as_user(user); end

    # pkg:gem/audited#lib/audited/audit.rb:58
    def ascending(*args, **_arg1); end

    # @private
    #
    # pkg:gem/audited#lib/audited/audit.rb:157
    def assign_revision_attributes(record, attributes); end

    # pkg:gem/audited#lib/audited/audit.rb:67
    def auditable_finder(*args, **_arg1); end

    # pkg:gem/audited#lib/audited/audit.rb:49
    def audited_class_names; end

    # pkg:gem/audited#lib/audited/audit.rb:49
    def audited_class_names=(val); end

    # Returns the list of classes that are being audited
    #
    # pkg:gem/audited#lib/audited/audit.rb:133
    def audited_classes; end

    # use created_at as timestamp cache key
    #
    # pkg:gem/audited#lib/audited/audit.rb:171
    def collection_cache_key(collection = T.unsafe(nil), *_arg1); end

    # pkg:gem/audited#lib/audited/audit.rb:60
    def creates(*args, **_arg1); end

    # pkg:gem/audited#lib/audited/audit.rb:59
    def descending(*args, **_arg1); end

    # pkg:gem/audited#lib/audited/audit.rb:62
    def destroys(*args, **_arg1); end

    # pkg:gem/audited#lib/audited/audit.rb:65
    def from_version(*args, **_arg1); end

    # pkg:gem/audited#lib/audited/audit.rb:42
    def page(num = T.unsafe(nil)); end

    # @private
    #
    # pkg:gem/audited#lib/audited/audit.rb:149
    def reconstruct_attributes(audits); end

    # pkg:gem/audited#lib/audited/audit.rb:66
    def to_version(*args, **_arg1); end

    # pkg:gem/audited#lib/audited/audit.rb:64
    def up_until(*args, **_arg1); end

    # pkg:gem/audited#lib/audited/audit.rb:61
    def updates(*args, **_arg1); end

    private

    # pkg:gem/audited#lib/audited/audit.rb:43
    def __class_attr___callbacks; end

    # pkg:gem/audited#lib/audited/audit.rb:43
    def __class_attr___callbacks=(new_value); end

    # pkg:gem/audited#lib/audited/audit.rb:43
    def __class_attr__reflections; end

    # pkg:gem/audited#lib/audited/audit.rb:43
    def __class_attr__reflections=(new_value); end

    # pkg:gem/audited#lib/audited/audit.rb:42
    def __class_attr__validators; end

    # pkg:gem/audited#lib/audited/audit.rb:42
    def __class_attr__validators=(new_value); end

    # pkg:gem/audited#lib/audited/audit.rb:42
    def __class_attr_defined_enums; end

    # pkg:gem/audited#lib/audited/audit.rb:42
    def __class_attr_defined_enums=(new_value); end
  end
end

# pkg:gem/audited#lib/audited/audit.rb:42
module Audited::Audit::GeneratedAssociationMethods
  # pkg:gem/audited#lib/audited/audit.rb:45
  def associated; end

  # pkg:gem/audited#lib/audited/audit.rb:45
  def associated=(value); end

  # pkg:gem/audited#lib/audited/audit.rb:45
  def associated_changed?; end

  # pkg:gem/audited#lib/audited/audit.rb:45
  def associated_previously_changed?; end

  # pkg:gem/audited#lib/audited/audit.rb:43
  def auditable; end

  # pkg:gem/audited#lib/audited/audit.rb:43
  def auditable=(value); end

  # pkg:gem/audited#lib/audited/audit.rb:43
  def auditable_changed?; end

  # pkg:gem/audited#lib/audited/audit.rb:43
  def auditable_previously_changed?; end

  # pkg:gem/audited#lib/audited/audit.rb:45
  def reload_associated; end

  # pkg:gem/audited#lib/audited/audit.rb:43
  def reload_auditable; end

  # pkg:gem/audited#lib/audited/audit.rb:44
  def reload_user; end

  # pkg:gem/audited#lib/audited/audit.rb:45
  def reset_associated; end

  # pkg:gem/audited#lib/audited/audit.rb:43
  def reset_auditable; end

  # pkg:gem/audited#lib/audited/audit.rb:44
  def reset_user; end

  # pkg:gem/audited#lib/audited/audit.rb:44
  def user; end

  # pkg:gem/audited#lib/audited/audit.rb:44
  def user=(value); end

  # pkg:gem/audited#lib/audited/audit.rb:44
  def user_changed?; end

  # pkg:gem/audited#lib/audited/audit.rb:44
  def user_previously_changed?; end
end

# pkg:gem/audited#lib/audited/audit.rb:42
module Audited::Audit::GeneratedAttributeMethods; end

# Specify this act if you want changes to your model to be saved in an
# audit table.  This assumes there is an audits table ready.
#
#   class User < ActiveRecord::Base
#     audited
#   end
#
# To store an audit comment set model.audit_comment to your comment before
# a create, update or destroy operation.
#
# See <tt>Audited::Auditor::ClassMethods#audited</tt>
# for configuration options
#
# pkg:gem/audited#lib/audited/auditor.rb:16
module Audited::Auditor
  extend ::ActiveSupport::Concern

  mixes_in_class_methods ::Audited::Auditor::ClassMethods
end

# pkg:gem/audited#lib/audited/auditor.rb:454
module Audited::Auditor::AuditedClassMethods
  # All audit operations during the block are recorded as being
  # made by +user+. This is not model specific, the method is a
  # convenience wrapper around
  # @see Audit#as_user.
  #
  # pkg:gem/audited#lib/audited/auditor.rb:510
  def audit_as(user, &block); end

  # Returns an array of columns that are audited. See non_audited_columns
  #
  # pkg:gem/audited#lib/audited/auditor.rb:456
  def audited_columns; end

  # pkg:gem/audited#lib/audited/auditor.rb:514
  def auditing_enabled; end

  # pkg:gem/audited#lib/audited/auditor.rb:518
  def auditing_enabled=(val); end

  # pkg:gem/audited#lib/audited/auditor.rb:522
  def default_ignored_attributes; end

  # pkg:gem/audited#lib/audited/auditor.rb:498
  def disable_auditing; end

  # pkg:gem/audited#lib/audited/auditor.rb:502
  def enable_auditing; end

  # We have to calculate this here since column_names may not be available when `audited` is called
  #
  # pkg:gem/audited#lib/audited/auditor.rb:461
  def non_audited_columns; end

  # pkg:gem/audited#lib/audited/auditor.rb:465
  def non_audited_columns=(columns); end

  # Executes the block with auditing enabled.
  #
  #   Foo.with_auditing do
  #     @foo.save
  #   end
  #
  # pkg:gem/audited#lib/audited/auditor.rb:490
  def with_auditing; end

  # Executes the block with auditing disabled.
  #
  #   Foo.without_auditing do
  #     @foo.save
  #   end
  #
  # pkg:gem/audited#lib/audited/auditor.rb:476
  def without_auditing; end

  protected

  # pkg:gem/audited#lib/audited/auditor.rb:536
  def calculate_non_audited_columns; end

  # pkg:gem/audited#lib/audited/auditor.rb:546
  def class_auditing_enabled; end

  # pkg:gem/audited#lib/audited/auditor.rb:528
  def normalize_audited_options; end

  # pkg:gem/audited#lib/audited/auditor.rb:550
  def reset_audited_columns; end
end

# pkg:gem/audited#lib/audited/auditor.rb:121
module Audited::Auditor::AuditedInstanceMethods
  # List of attributes that are audited.
  #
  # pkg:gem/audited#lib/audited/auditor.rb:190
  def audited_attributes; end

  # Combine multiple audits into one.
  #
  # pkg:gem/audited#lib/audited/auditor.rb:205
  def combine_audits(audits_to_combine); end

  # Returns a list combined of record audits and associated audits.
  #
  # pkg:gem/audited#lib/audited/auditor.rb:198
  def own_and_associated_audits; end

  # Get a specific revision specified by the version number, or +:previous+
  # Returns nil for versions greater than revisions count
  #
  # pkg:gem/audited#lib/audited/auditor.rb:177
  def revision(version); end

  # Find the oldest revision recorded prior to the date/time provided.
  #
  # pkg:gem/audited#lib/audited/auditor.rb:184
  def revision_at(date_or_time); end

  # Gets an array of the revisions available
  #
  #   user.revisions.each do |revision|
  #     user.name
  #     user.version
  #   end
  #
  # pkg:gem/audited#lib/audited/auditor.rb:161
  def revisions(from_version = T.unsafe(nil)); end

  # Temporarily turns on auditing while saving.
  #
  # pkg:gem/audited#lib/audited/auditor.rb:140
  def save_with_auditing; end

  # Temporarily turns off auditing while saving.
  #
  # pkg:gem/audited#lib/audited/auditor.rb:125
  def save_without_auditing; end

  # Executes the block with the auditing callbacks enabled.
  #
  #   @foo.with_auditing do
  #     @foo.save
  #   end
  #
  # pkg:gem/audited#lib/audited/auditor.rb:150
  def with_auditing(&block); end

  # Executes the block with the auditing callbacks disabled.
  #
  #   @foo.without_auditing do
  #     @foo.save
  #   end
  #
  # pkg:gem/audited#lib/audited/auditor.rb:135
  def without_auditing(&block); end

  protected

  # pkg:gem/audited#lib/audited/auditor.rb:223
  def revision_with(attributes); end

  private

  # pkg:gem/audited#lib/audited/auditor.rb:349
  def audit_create; end

  # pkg:gem/audited#lib/audited/auditor.rb:430
  def audit_create_callback; end

  # pkg:gem/audited#lib/audited/auditor.rb:368
  def audit_destroy; end

  # pkg:gem/audited#lib/audited/auditor.rb:430
  def audit_destroy_callback; end

  # pkg:gem/audited#lib/audited/auditor.rb:361
  def audit_touch; end

  # pkg:gem/audited#lib/audited/auditor.rb:354
  def audit_update; end

  # pkg:gem/audited#lib/audited/auditor.rb:430
  def audit_update_callback; end

  # pkg:gem/audited#lib/audited/auditor.rb:250
  def audited_changes(for_touch: T.unsafe(nil), exclude_readonly_attrs: T.unsafe(nil)); end

  # pkg:gem/audited#lib/audited/auditor.rb:433
  def auditing_enabled; end

  # pkg:gem/audited#lib/audited/auditor.rb:337
  def audits_to(version = T.unsafe(nil)); end

  # pkg:gem/audited#lib/audited/auditor.rb:402
  def combine_audits_if_needed; end

  # pkg:gem/audited#lib/audited/auditor.rb:395
  def comment_required_state?; end

  # pkg:gem/audited#lib/audited/auditor.rb:411
  def evaluate_max_audits; end

  # Replace values for given attrs to a placeholder and return modified hash
  #
  # @param audited_changes [Hash] Hash of changes to be saved to audited version record
  # @param attrs [Array<String>] Array of attrs, values of which will be replaced to placeholder value
  # @param placeholder [String] Placeholder to replace original attr values
  #
  # pkg:gem/audited#lib/audited/auditor.rb:320
  def filter_attr_values(audited_changes: T.unsafe(nil), attrs: T.unsafe(nil), placeholder: T.unsafe(nil)); end

  # pkg:gem/audited#lib/audited/auditor.rb:308
  def filter_encrypted_attrs(filtered_changes); end

  # pkg:gem/audited#lib/audited/auditor.rb:282
  def normalize_enum_changes(changes); end

  # pkg:gem/audited#lib/audited/auditor.rb:389
  def presence_of_audit_comment; end

  # pkg:gem/audited#lib/audited/auditor.rb:333
  def rails_below?(rails_version); end

  # pkg:gem/audited#lib/audited/auditor.rb:447
  def reconstruct_attributes(audits); end

  # pkg:gem/audited#lib/audited/auditor.rb:300
  def redact_values(filtered_changes); end

  # pkg:gem/audited#lib/audited/auditor.rb:422
  def require_comment; end

  # pkg:gem/audited#lib/audited/auditor.rb:439
  def run_conditional_check(condition, matching: T.unsafe(nil)); end

  # pkg:gem/audited#lib/audited/auditor.rb:375
  def write_audit(attrs); end
end

# pkg:gem/audited#lib/audited/auditor.rb:122
Audited::Auditor::AuditedInstanceMethods::REDACTED = T.let(T.unsafe(nil), String)

# pkg:gem/audited#lib/audited/auditor.rb:19
Audited::Auditor::CALLBACKS = T.let(T.unsafe(nil), Array)

# pkg:gem/audited#lib/audited/auditor.rb:21
module Audited::Auditor::ClassMethods
  # * +redacted+ - Changes to these fields will be logged, but the values
  #   will not. This is useful, for example, if you wish to audit when a
  #   password is changed, without saving the actual password in the log.
  #   To store values as something other than '[REDACTED]', pass an argument
  #   to the redaction_value option.
  #
  #     class User < ActiveRecord::Base
  #       audited redacted: :password, redaction_value: SecureRandom.uuid
  #     end
  #
  # * +if+ - Only audit the model when the given function returns true
  # * +unless+ - Only audit the model when the given function returns false
  #
  #     class User < ActiveRecord::Base
  #       audited :if => :active?
  #
  #       def active?
  #         self.status == 'active'
  #       end
  #     end
  #
  # pkg:gem/audited#lib/audited/auditor.rb:61
  def audited(options = T.unsafe(nil)); end

  private

  # pkg:gem/audited#lib/audited/auditor.rb:67
  def audited?; end

  # pkg:gem/audited#lib/audited/auditor.rb:104
  def has_associated_audits; end

  # pkg:gem/audited#lib/audited/auditor.rb:71
  def set_audit(options); end

  # pkg:gem/audited#lib/audited/auditor.rb:114
  def set_audited_options(options); end

  # pkg:gem/audited#lib/audited/auditor.rb:108
  def update_audited_options(new_options); end
end

# pkg:gem/audited#lib/audited/railtie.rb:4
class Audited::Railtie < ::Rails::Railtie; end

# Wrapper around ActiveSupport::CurrentAttributes
#
# pkg:gem/audited#lib/audited.rb:7
class Audited::RequestStore < ::ActiveSupport::CurrentAttributes
  class << self
    # pkg:gem/audited#lib/audited.rb:8
    def audited_store; end

    # pkg:gem/audited#lib/audited.rb:8
    def audited_store=(value); end

    private

    # pkg:gem/audited#lib/audited.rb:8
    def __class_attr_defaults; end

    # pkg:gem/audited#lib/audited.rb:8
    def __class_attr_defaults=(new_value); end
  end
end

# pkg:gem/audited#lib/audited/sweeper.rb:4
class Audited::Sweeper
  # pkg:gem/audited#lib/audited/sweeper.rb:13
  def around(controller); end

  # pkg:gem/audited#lib/audited/sweeper.rb:34
  def controller; end

  # pkg:gem/audited#lib/audited/sweeper.rb:38
  def controller=(value); end

  # pkg:gem/audited#lib/audited/sweeper.rb:22
  def current_user; end

  # pkg:gem/audited#lib/audited/sweeper.rb:26
  def remote_ip; end

  # pkg:gem/audited#lib/audited/sweeper.rb:30
  def request_uuid; end

  # pkg:gem/audited#lib/audited/sweeper.rb:11
  def store(&_arg0); end
end

# pkg:gem/audited#lib/audited/sweeper.rb:5
Audited::Sweeper::STORED_DATA = T.let(T.unsafe(nil), Hash)

# Audit saves the changes to ActiveRecord models.  It has the following attributes:
#
# * <tt>auditable</tt>: the ActiveRecord model that was changed
# * <tt>user</tt>: the user that performed the change; a string or an ActiveRecord model
# * <tt>action</tt>: one of create, update, or delete
# * <tt>audited_changes</tt>: a hash of all the changes
# * <tt>comment</tt>: a comment set with the audit
# * <tt>version</tt>: the version of the model
# * <tt>request_uuid</tt>: a uuid based that allows audits from the same controller request
# * <tt>created_at</tt>: Time that the change was performed
#
# pkg:gem/audited#lib/audited/audit.rb:18
class Audited::YAMLIfTextColumnType
  class << self
    # pkg:gem/audited#lib/audited/audit.rb:28
    def dump(obj); end

    # pkg:gem/audited#lib/audited/audit.rb:20
    def load(obj); end

    # pkg:gem/audited#lib/audited/audit.rb:36
    def text_column?; end
  end
end
